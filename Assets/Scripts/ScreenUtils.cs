...
public static class ScreenUtils
{
	...
	{
		// save screen edges in world coordinates
		Vector2 lowerLeftCornerScreen = new Vector2(0, 0);
		Vector2 upperRightCornerScreen = new Vector2((float)Screen.width, (float)Screen.height);
		Vector2 lowerLeftCornerWorld = Camera.main.ScreenToWorldPoint(lowerLeftCornerScreen);
		Vector2 upperRightCornerWorld = Camera.main.ScreenToWorldPoint(upperRightCornerScreen);
		screenLeft = lowerLeftCornerWorld.x;
		screenRight = upperRightCornerWorld.x;
		screenTop = upperRightCornerWorld.y;
		screenBottom = lowerLeftCornerWorld.y;
